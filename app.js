...
async function saveTracker(e){e.preventDefault();const id=$('#trackerId').value;const payload={id:id||crypto.randomUUID(),user_id:session.user.id,name:$('#trackerName').value.trim(),unit:$('#trackerUnit').value.trim(),icon:$('#trackerIcon').value.trim()||'◫',color:$('#trackerColor').value,daily_goal:$('#trackerGoal').value===''?null:Number($('#trackerGoal').value),quick_values:$('#trackerQuickValues').value.split(',').map(v=>Number(v.trim())).filter(v=>v>0)};if(!payload.quick_values.length)return toast('Add at least one quick value');await mutate(id?'updateTracker':'insertTracker',{id,payload});$('#trackerDialog').close();await loadData()}
...
function applyOptimistic(type,{id,payload}){if(type==='insertTracker')state.trackers.push({...payload,created_at:new Date().toISOString(),updated_at:new Date().toISOString(),is_active:true,sort_order:0});if(type==='insertLog')state.logs.unshift({...payload,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});if(type==='updateTracker')state.trackers=state.trackers.map(x=>x.id===id?{...x,...payload}:x);if(type==='updateLog')state.logs=state.logs.map(x=>x.id===id?{...x,...payload}:x);if(type==='deleteTracker')state.trackers=state.trackers.filter(x=>x.id!==id);if(type==='deleteLog')state.logs=state.logs.filter(x=>x.id!==id);cacheState();renderAll()}
...
window.quickLog=async(id,value)=>{await mutate('insertLog',{payload:{id:crypto.randomUUID(),user_id:session.user.id,tracker_id:id,value,occurred_at:new Date().toISOString(),note:null,source:'website'}});await loadData()};
